...
// to simplify the task, We use only 1 market and 1 Base Currency
...
export class Engine{
    ...
    constructor() {
        ...
        // inital snapshot of the orderbook for dummy data
        try {
        ...
        if (snapshot) {
            const snapshotSnapshot = JSON.parse(snapshot.toString());   // take the dummy data from the snapshot.json file
    ...
    // this is main function of our trade engine to process all the operation
    // from the api servver we send a message like order place, cancel order, get depth ... 

    // proccess the task after that return the output thorugh pub/subs to api server
        ...
        switch (message.type) {
            ...
            case CREATE_ORDER:
                try {
                    ...
                    RedisManager.getInstance().sendToApi(clientId, {      // send to api server thorugh pub/subs    
                        type: "ORDER_PLACED",
                        payload: {
                            orderId,
                            executedQty,
    ...
    // update the order in the database message queue
    ...
    // store the trade in DaaBase message queue
    ...
    // publish the trade to the webSocket server through pub/subs
    ...
    // publish the data to webSocket server through pub/subs
    ...
    // update the balance of the user after the order is placed
    // fills are the trades that are executed after the order is placed( sell order is executed by the buy order and vice versa)
    ...
    // check the balance of the user before placing the order
    // if the user has sufficient balance then lock the funds for the order
    ...
    // set the base balance for the dummy data
    setBaseBalances() {
        this.balances.set("1", {
            [BASE_CURRENCY]: {
                available: 10000000,
